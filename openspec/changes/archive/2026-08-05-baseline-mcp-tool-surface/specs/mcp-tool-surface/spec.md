...
**Known deviation at baseline.** One input escapes this requirement today: an id whose only invalid character is a trailing newline passes validation, because the validators anchor with `$` rather than `\Z`. It then fails inside the HTTP layer as `httpx.InvalidURL`, which the tool does not catch — the framework wraps it, so the caller receives a tool error of the right *type* carrying the wrong *message*: `Invalid non-printable ASCII character in URL` instead of a statement of the accepted id form. Nothing is sent to Aleph, so this is an error-translation defect and not a read-only defect. It is recorded as a strict-expected-failure in the test suite so that fixing it fails the suite and forces this note to be removed. The requirement is stated as the contract, not narrowed to match the defect.

...
- [ ] 1.1 Extract the `Retry-After` read out of `_retry_delay` in
...
- [ ] 1.2 Add the two inputs to `raise_for_status` in `src/aleph_mcp/errors.py` — whether
...
- [ ] 1.3 Compose the clause: retryability, attempt count, and one of three mutually
...
- [ ] 1.4 Call the parser on the terminal response at the `give_up` branch
...
- [ ] 1.5 Verify the reported value is the advertised value normalised by the ceiling
...
- [ ] 1.6 Confirm a non-retryable status gains nothing: verify a `404` and a `403` message
  are byte-identical to their current text.
- [ ] 1.7 Confirm the existing `budget_spent` clause still appears, and reads coherently
  beside the new one rather than repeating the attempt count.

## 2. Identifier source clause

- [ ] 2.1 Add a per-field source clause to `_check_entity_id` in
...
- [ ] 2.2 Verify the clause is unconditional: refuse two values for one field, one
  resembling a rendered label and one not, and confirm both messages carry the same clause.
- [ ] 2.3 Verify the charset, the echo of the rejected value, and the addresses-nothing
...
- [ ] 3.2 Re-read every existing test asserting on these two messages. Error wording is the
...
- [ ] 3.3 Verify no test asserts a wait value where the fixture's final response carried no
  `Retry-After` — that would encode the invented-wait behaviour this change refuses.
- [ ] 3.4 Cover the earlier-attempt case: a fixture whose first response advertises a wait
